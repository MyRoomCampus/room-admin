...
// 封装了数组基本操作的hook
export const useArray = <T>(initialList: T[] = []): [T[], typeof operations] => {
  ...
  const operations = {
    ...
    empty: () => set([]),
    filter: (filterFn: (item: T) => boolean) => set((list) => list.filter(filterFn)),
    remove: (item: T) => {
    ...
    setAt: (index: number, value: T) => set((list) => [...list.slice(0, index), value, ...list.slice(index + 1)]),
    removeAt: (index: number) => set((list) => [...list.slice(0, index), ...list.slice(index + 1)]),
